...
func runStakersTest(t *testing.T, faultConfig challenge.FaultConfig, maxGasPerNode *big.Int, expectedEnd ExpectedChallengeEnd) {
	...
	valAuth, err := ethbridge.NewTransactAuth(ctx, client, auth, &configuration.Config{})
	test.FailIfError(t, err)
	val2Auth, err := ethbridge.NewTransactAuth(ctx, client, auth2, &configuration.Config{})
	test.FailIfError(t, err)
